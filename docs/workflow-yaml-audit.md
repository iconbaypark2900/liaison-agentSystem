...
|------|---------------------------|-------------|-------------------|-------|
| [workflows/sigma-integration.yaml](../workflows/sigma-integration.yaml) | `sigma` (`workflow_source` in plan) | 4 (`archive_inspect`, `module_validate`, `review`, `closeout`) | **Actionable** | Full phase structure with routes, skills, artifacts, quality gates. `validation_profile: sigma` → `checks/sigma.sh`. |
